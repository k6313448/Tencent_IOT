...
void on_receive_data_point_internal(unsigned long long from_client, tx_data_point * data_points, int data_points_count)
{
	...
	for(i=0; i<data_points_count; ++i){
		...
		)
		{
            ...
            else if(data_points[i].id == 10001)
            {
                unsigned int cookie = 0;
                tx_data_point dp_to_send = {0};
				char ip[20];
            ...
            else if(data_points[i].id == 10002)
            {
                unsigned int cookie = 0;
                tx_data_point dp_to_send = {0};
				int interval=0;
...
bool initDevice() {
	...
	int nLicenseSize = 0;
	if (!readBufferFromFile("./licence.txt", license, sizeof(license), &nLicenseSize)) {
    ...
    int nPubkeySize = 0;
    if (!readBufferFromFile("./1700003134.pem", svrPubkey, sizeof(svrPubkey), &nPubkeySize))
    {
